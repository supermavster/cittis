...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "form1")) {
  $insertSQL = sprintf("INSERT INTO peaje (Nombre, CategoriaPeaje, IdAp, IdOp) VALUES (%s, %s, %s, %s)",
                       GetSQLValueString($_POST['Nombre'], "text"),
                       GetSQLValueString($_POST['CategoriaPeaje'], "text"),
                       GetSQLValueString($_POST['IdAp'], "int"),
                       GetSQLValueString($_POST['IdOp'], "int"));

  mysql_select_db($database_Conexion, $Conexion);
  $Result1 = mysql_query($insertSQL, $Conexion) or die(mysql_error());

  $insertGoTo = "Peaje.php";
  if (isset($_SERVER['QUERY_STRING'])) {
...
mysql_select_db($database_Conexion, $Conexion);
$query_Recordsetrecursos = "SELECT * FROM adminrecursos";
$Recordsetrecursos = mysql_query($query_Recordsetrecursos, $Conexion) or die(mysql_error());
$row_Recordsetrecursos = mysql_fetch_assoc($Recordsetrecursos);
$totalRows_Recordsetrecursos = mysql_num_rows($Recordsetrecursos);

mysql_select_db($database_Conexion, $Conexion);
$query_RecordsetOperador = "SELECT * FROM operador";
$RecordsetOperador = mysql_query($query_RecordsetOperador, $Conexion) or die(mysql_error());
$row_RecordsetOperador = mysql_fetch_assoc($RecordsetOperador);
$totalRows_RecordsetOperador = mysql_num_rows($RecordsetOperador);
			  ...
			  <span class="menu"></span>
				  <div class="top-menu">
							 <ul class="nav navbar-nav cl-effect-14">
								...
								<li><a href="Pesaje.php" class="scroll">Agregar Pesaje</a></li>
...
<menu class="clearfix">
    ...
    <ul class="main-menu clearfix">
        ...
        <li class="search fa fa-search"></li>
            ...
            <br>
            <br>
            <br><h1><span>Peaje</span></h1>
            <br>
            <br>
            <br>
            <img src="images/Peajes.jpg" width="354" height="179"/>
            <br>

<form method="post" name="form1" action="<?php echo $editFormAction; ?>">
  <table align="center">
   <tr valign="baseline">
      <td height="121" align="right" nowrap>Inventario Vial:</td>
      <td><input type="text" name="IdIv" value="<?php echo $row['IdIv']; ?>" readonly size="32"/></td>
    </tr>
    <tr valign="baseline">
      <td height="103" align="right" nowrap>Nombre:</td>
      <td><input type="text" name="Nombre" value="" size="32"></td>
    </tr>
    <tr valign="baseline">
      <td height="83" align="right" nowrap>CategoriaPeaje:</td>
      <td><input type="text" name="CategoriaPeaje" value="" size="32"></td>
    </tr>
    <tr valign="baseline">
      <td height="87" align="right" nowrap>Administrador:</td>
      <td><select name="IdAp">
         <option value="Seleccione">--Seleccione una opcion--</option>
        <?php 
do {  
?>
        <option value="<?php echo $row_Recordsetrecursos['IdAp']?>" ><?php echo $row_Recordsetrecursos['Nombre']?></option>
        <?php
} while ($row_Recordsetrecursos = mysql_fetch_assoc($Recordsetrecursos));
?>
      </select></td>
    <tr>
    <tr valign="baseline">
      <td height="67" align="right" nowrap>Operador:</td>
      <td><select name="IdOp">
         <option value="Seleccione">--Seleccione una opcion--</option>
        <?php 
do {  
?>
        <option value="<?php echo $row_RecordsetOperador['IdOp']?>" ><?php echo $row_RecordsetOperador['Nombre']?></option>
        <?php
} while ($row_RecordsetOperador = mysql_fetch_assoc($RecordsetOperador));
?>
      </select></td>
    <tr>
    <tr valign="baseline">
...
<p>&nbsp;</p>
<?php
mysql_free_result($Recordsetrecursos);

mysql_free_result($RecordsetOperador);
?>